# Revised publication-style version of plot_K_irrepresentability.R
# Based on the uploaded script, with cleaner styling for paper-quality figures.
...
out_dir <- "raw_experiments/irrepresentability/plots"
...
is_K_positive_definite <- c(
  is_KAR_positive_definite,
  is_Khub_positive_definite
)[[which_graph]]
...
irrep_axis_breaks <- function(all_a, all_c, a_every = 40, c_breaks = NULL) {
  y_breaks <- all_a[seq(1, length(all_a), by = a_every)]
  if (tail(y_breaks, 1) != tail(all_a, 1)) {
    y_breaks <- c(y_breaks, tail(all_a, 1))
  }
...
plot_irrep_publication <- function(mat,
  ...
  ggplot(df, aes(x = c, y = a, fill = value)) +
    ...
    theme(
      ...
      axis.title.y = element_text(angle = 0, vjust = 0.5,margin = margin(r = 1)),
...
p_both_pub <- plot_irrep_pair_publication(
  ...
  pcglasso_limits = c(0, max(my_res_PCGLASSO, na.rm = TRUE)),
  midpoint = 1,
  out_file = file.path(out_dir, paste0("irrep_on_", Ktext, "_v2.png"))
)

grid::grid.newpage()
grid::grid.draw(p_both_pub)
